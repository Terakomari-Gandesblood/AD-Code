...
class Agent:
    def __init__(self):
        ...
        self.old_actor = ActorNetwork.create_from_existing(another_net=self.actor).to(self.device)
    ...
    def build_hetero_graph(self, feats: EncodedFeatureDict, raw_state: RawState) -> HeteroData:
        ...
        :param raw_state: 用于K近邻 RawState.surrounding_vehicles: Optional[SurroundingVehicles] = None
        :param feats: 节点编码特征
        :return: 异构图
        """
        device = self.device
        ...
        # 定义节点
        data['ego'].x = feats['ego'].to(device)  # (1, D)
        data['gnss'].x = feats['gnss'].to(device)  # (1, D)
        # data['imu'].x = feats['imu'].to(device)  # (1, D)
        data['traffic_light'].x = feats['traffic_light'].to(device)  # (1, D)
        data['lane'].x = feats['lane'].to(device)  # (1, D)

        has_vehicles = feats['vehicles'] is not None
        if has_vehicles:
            vehicles_x = feats['vehicles'].to(device)  # (N, D)
            N = vehicles_x.size(0)
        ...
        # ego <-> imu
        # data[('ego', 'to', 'imu')].edge_index = torch.tensor([[0], [0]], dtype=torch.long, device=device)
        # data[('imu', 'to', 'ego')].edge_index = torch.tensor([[0], [0]], dtype=torch.long, device=device)
        ...
        if has_vehicles and N > 0:
            ...
            if M > 0:
                ...
                diff = rel_xy.unsqueeze(1) - rel_xy.unsqueeze(0)  # (M, M, 2)
                dists = torch.norm(diff, dim=-1)  # (M, M)

                k = 3

                k_eff = min(k + 1, M)
                knn_idx = torch.topk(-dists, k=k_eff, dim=-1).indices  # 找到最近的邻居

                src_list, dst_list = [], []
                for i in range(M):
                    neigh = knn_idx[i].tolist()
                    neigh = [j for j in neigh if j != i][:k]  # 去掉自身
                    if len(neigh) == 0:
                        continue
                ...
                if len(src_list) > 0:
                    src_all = torch.cat(src_list, dim=0)  # (E,)
                    dst_all = torch.cat(dst_list, dim=0)  # (E,)
    ...
    def get_global_feat(self, feats: EncodedFeatureDict, raw_state: RawState) -> torch.Tensor:
        ...
        # 卷积和池化
        return self.gcn(graph)

    def select_action(self,
                      global_feat: torch.Tensor,  # 全局特征用于高层网络做全局策略
                      obs: torch.Tensor,  # 局部特征用于底层网络做ego控制
                      yaw_err,  # 转向误差
                      v_err,  # 速度误差
        ...
        with torch.no_grad():
            ...
            )  # info包括高层网络的决策以及高层的逻辑分布、低层网络的决策以及逻辑分布和动作及模板
    ...
    def update_actor(self,
        ...
        # 策略裁剪
        surr1 = ratio * adv  # 未裁剪的目标
        surr2 = torch.clamp(ratio, 1.0 - epsilon, 1.0 + epsilon) * adv  # 裁剪后的目标
        policy_loss = -torch.min(surr1, surr2).mean()  # 取两者最小值
    ...
    def update_critic(self,
        ...
        # 当前与下一时刻的价值
        values = self.critic(states.to(device))  # (B,)

        with torch.no_grad():
            next_v = self.critic(next_states.to(device))  # (B,)
        ...
        if value_clip is not None:
            ...
            v_clipped = v_old + (values - v_old).clamp(-value_clip, value_clip)  # 限制更新幅度
            # 价值裁剪
            loss_un_clipped = (values - targets).pow(2)  # 标准MSE
            loss_clipped = (v_clipped - targets).pow(2)  # 裁剪后的MSE
            value_loss = torch.max(loss_un_clipped, loss_clipped).mean()  # 取两者最大值
        else:
    ...
    def update_all_net(self,
                       batch_trajectories,  # 搜集的轨迹(states、actions、rewards...)
                       batch_advantages  # 计算的广义优势估计
                       ):
        device = self.device
