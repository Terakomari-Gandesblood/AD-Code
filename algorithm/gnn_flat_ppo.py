...
@torch.no_grad()
def collect_trajectories(agent: AgentFlat, env: CarlaEnv, max_steps: int) -> List[dict]:
    ...
    while step_count < max_steps:
        ...
        next_critic_state = global_feat_next
...
def compute_advantages(rewards: List[float],
    ...
    for t in reversed(range(T)):
        ...
        delta = rewards[t] + gamma * values[t + 1] * nonterminal - values[t]  # TD误差
        last_gae = delta + gamma * lam * nonterminal * last_gae  # GAE递归
        adv[t] = last_gae
...
def train():
    ...
    if getattr(config, "gnn_flat_ppo_resume_filename", None):
        try:
    ...
    for episode in range(config.episodes):
        ...
        if (episode + 1) % getattr(config, "save_every", 5) == 0:
            agent.save_checkpoint(episode, {"avg_reward": avg_reward, "ep_reward": episode_reward},
                                  filename=f"gnn_flat_ppo_ep{episode}.pt")

        if avg_reward > best_avg_reward:
            best_avg_reward = avg_reward
            agent.save_checkpoint(episode, {"avg_reward": avg_reward, "ep_reward": episode_reward},
                                  filename="gnn_flat_ppo_best.pt")
