...
from ablation.pure_policy.pure_actor import PureActorNetwork
...
class PureAgent:
    def __init__(self):
        ...
        self.actor = PureActorNetwork().to(self.device)
        self.old_actor = PureActorNetwork().to(self.device)
    ...
    def select_action(self,
        ...
        out = {
            "squashed": info["squashed"].detach(),
            "log_p_old": log_p_old.detach(),
        }
    ...
    def save_checkpoint(self, episode: int, metrics: dict, filename: str):
        save_dir = config.pure_ppo_save_path
