...
class HierarchicalPPOEvalPolicy:
    ...
    def act(self, env: CarlaEnv) -> Tuple[float, float, float]:
        ...
        # 2) 高层：用各 encoder 的输出拼 global_feat
...
def run_eval(
    ...
    ckpt_filename: str = "hierarchical_ppo_best.pt",
):
    # 1) 环境
    env = CarlaEnv(no_rendering_mode=no_rendering_mode, npc_vehicle_num=npc_vehicle_num)

    # 2) 策略
    policy = HierarchicalPPOEvalPolicy(ckpt_filename=ckpt_filename)
...
if __name__ == "__main__":
    run_eval(
        ...
        npc_vehicle_num=10,
        ckpt_filename="hierarchical_ppo_best.pt",
    )
